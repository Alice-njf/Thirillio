...
import com.cadence.thrillio.constants.Gender;
...
public class DataStore {
	...
	private static void loadBooks() {
		...
		for(String row : data) {
			...
			bookmarks[2][rowNum++] = BookmarkManager.getInstance().createBook(Long.parseLong(values[0]), values[1], values[2], Integer.parseInt(values[3]), values[4], authors, values[6], Double.parseDouble(values[7]));
	...
	private static void loadMovies() {
		...
		bookmarks[1][4] = BookmarkManager.getInstance().createMovie(3004, "Ikiru", "", 1952,
				new String[] { "Takashi Shimura,Minoru Chiaki Akira" }, new String[] { "Kurosawa Foreign" },
		...
		for(String row : data) {
			String[] values = row.split("\t");
			String[] cast = values[4].split(",");
			String[] directores = values[5].split(",");
			bookmarks[1][rowNum++] = BookmarkManager.getInstance().createMovie(Long.parseLong(values[0]), values[1],  values[2], Integer.parseInt(values[3]),cast, directores, values[6], Double.parseDouble(values[7]));
		}
	...
	private static void loadUsers() {
		...
		users[4] = UserManager.getInstance().createUser(1004, "user4@semanticsquare.com", "test", "Dheeru", "M",
				Gender.MALE, UserType.CHIEFEDITOR);*/
	...
	private static void loadWeblinks() {
		String[] data = new String[USER_BOOKMARK_LIMIT];
		IOUtil.read(data, "WebLink");
		int rowNum = 0;
		for(String row : data){
			String[] values = row.split("\t");
			
			bookmarks [0][rowNum++] = BookmarkManager.getInstance().createWebLink(Long.parseLong(values[0]), values[1], values[2], values[2], values[4]);
		}
		/*
